...
React.useEffect(()=>{
 const results = GetEmployee.filter(person =>{
     if(person.role==='lead' || person.role==='hr' && window.location.hostname.toLowerCase().includes(person.company.toLowerCase())  && window.location.hostname.toLowerCase().includes(person.company.toLowerCase())){
       return true
     }
     else
     {
